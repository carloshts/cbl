...
$dados = array();
$aux = array("Margem de Alugueis/Produto");
...
$qtdgeral = 0;
    if($kp["nome_kit_produtos"] != 'Nulo'){
        $alugados = $KitProdutosDAO->PesquisarKitProdutosAlugadoGrafico($KitProdutosDTO, $mes);
        $qtdalugados = count($alugados);
        $todos = $KitProdutosDAO->PesquisarDiferenteDeNulo();
        $qtdgeral = count($todos);
        $margem = $qtdalugados / $qtdgeral  ;
...
$grafico->SetTitle("Margem de alugueis de Kits no ultimo mês");
// Tipo do gráfico
